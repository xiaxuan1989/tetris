...
void Rcrutch::revolve()
{
	//如果当前状态竖着，则旋转为横状态
	if (status == 1){
		if (block1.Y > 0 && block4.X + 1 < 12
			&& bg_status[block1.X - 1][block1.Y + 1] == BLANK
			&&bg_status[block4.X][block4.Y - 2] == BLANK) {
		...
		if (bg_status[block1.X - 1][block1.Y - 3] == BLANK
			&&bg_status[block2.X - 2][block2.Y - 2] == BLANK
			&&bg_status[block4.X - 2][block4.Y - 2] == BLANK){
